...
--- Radar UI --- 
local radarLines = {}
-- Lấy vị trí từ cấu hình cài đặt của bạn, nếu không có sẽ tự đặt ở (300, 250)
...
local function setRadarScale() -- cập nhật kích thước / độ zoom vòng tròn
...
do
    ...
    scriptCns.inputBegan = inputService.InputBegan:Connect(function(io) 
        ...
        if ( inputType == 'Keyboard' ) then
            ...
            elseif ( keyCode == 'LeftBracket' ) then -- Phím [ : Thu nhỏ Radar
                radarObjects.zoomText.Position = radarPosition + newV2(0, RADAR_RADIUS + 20)
                radarObjects.zoomText.Visible = true
            ...
            elseif ( keyCode == 'RightBracket' ) then -- Phím ] : Phóng to Radar
                radarObjects.zoomText.Position = radarPosition + newV2(0, RADAR_RADIUS + 20)
                radarObjects.zoomText.Visible = true
        ...
        -- 2. XỬ LÝ KÉO THẢ (PC MOUSE CHUỘT TRÁI HOẶC MOBILE TOUCH CẢM ỨNG)
        elseif ( inputType == 'MouseButton1' or inputType == 'Touch' ) then
            ...
            -- Kiểm tra xem vị trí chạm/click có nằm gọn trong vòng tròn Radar không
            if ( (startPos - radarPosition).Magnitude < RADAR_RADIUS ) then
                ...
                scriptCns.dragCn = inputService.InputChanged:Connect(function(i) 
                    if (i.UserInputType.Name == 'MouseMovement' and not currentTouchInput) or (i.UserInputType.Name == 'Touch' and i == currentTouchInput) then
                        ...
                        setRadarPosition(movePos) -- Di chuyển bám đuôi vị trí ngón tay/chuột ngay lập tức
                    end
                end)
            end
        end
...
local function updateRadarSize() -- Hàm cập nhật size radar khi chỉnh phím tắt
    ...
    setRadarScale() -- Vẽ lại vòng khoảng cách theo size mới
end
    ...
    scriptCns.inputChanged = inputService.InputChanged:Connect(function(input) 
        ...
        if ( nowTime - lastCheckTime > 0.03 and (inputType == 'MouseMovement' or inputType == 'Touch') ) then
            ...
            if ( (mousePos - radarPosition).Magnitude < RADAR_RADIUS ) then
                local distanceThresh = 20 -- Phạm vi nhận diện pixel xung quanh marker
                hoverPlayer = nil
